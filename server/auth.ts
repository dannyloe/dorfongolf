...
export const users = pgTable("users", {
  ...
  // Phase 4: canonical golfer profile — source of truth that pushes down to
  // each group's groupPlayers row (unless that group has overridden a field).
  handicapIndex: integer("handicap_index"), // stored as tenths (e.g., 124 = 12.4)
  teePreference: varchar("tee_preference"), // default tee: Black / Blue / White / Red
  discoverable: boolean("discoverable").notNull().default(true), // controls global player-search visibility
});
